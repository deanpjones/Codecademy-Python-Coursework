...
#FUNCTION: CREATE CIPHER (DYNAMICALLY)(CAESAR SHIFT)
# shift_direction - shift direction ('left' or 'right')
# shift_n
def create_cipher(shift_direction, shift_n):
    alphab = 'abcdefghijklmnopqrstuvwxyz'
    punc = ',\' !?.'
    if shift_direction == 'left':
        #cipher = alphab[1:] + alphab[:1] + punc
        cipher = alphab[shift_n:] + alphab[:shift_n] + punc
    elif shift_direction == 'right':
        #cipher = alphab[-1:] + alphab[:-1] + punc
        cipher = alphab[-shift_n:] + alphab[:-shift_n] + punc
    else:
        return 'Something went wrong (make sure arguments for shift are "left" or "right")'
    return cipher

#TEST
create_cipher('left', 3)        #'defghijklmnopqrstuvwxyzabc, !?.'
create_cipher('right', 10)      #'qrstuvwxyzabcdefghijklmnop, !?.'

#FUNCTION: ENCODE OR DECODE
# code = 'encode' or 'decode'
def encode_decode(my_string, shift_direction, shift_n, code):
    result = ''
    alphab = 'abcdefghijklmnopqrstuvwxyz,\' !?.'
    cipher = create_cipher(shift_direction, shift_n)
...
#TEST
encode_decode('xuo jxuhu!', 'right', 10, 'decode')          #'hey there!'
encode_decode('hey there!', 'right', 10, 'encode')          #'xuo jxuhu!'
